...
export const createSchoolServerController = async (
  ...
  const school = await createSchoolServerService(
    req.body,
...
export const retrieveSchoolClassController = async (
  req: Request,
  res: Response,
) => {
  const school = await retrieveSchoolClassService(
    req.params.school_id,
    req.params.year_id,
  );
  return res.json(school);
